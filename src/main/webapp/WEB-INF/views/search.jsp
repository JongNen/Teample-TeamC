			...
			<div>
				...
				<select name="doNm">
					...
					<c:forEach items="${datas }" var="obj">
						<option value="${obj.doNm }"
							${'param.obj' eq obj.doNm ? 'selected' : '' }>${obj.doNm }</option>
					</c:forEach>
				</select> <select name="sigunguNm">
					<option value="" ${param.sigunguNm eq '' ? 'selected' :  ''}>전체</option>
					<c:forEach items="${datas }" var="searchSigunguNm">
						<option value="${searchSigunguNm.sigunguNm }"
							${searchSigunguNm.sigunguNm eq parmam.sigunguNm ? 'selected' : ''}>${searchSigunguNm.sigunguNm }</option>
					</c:forEach>
				</select>
			</div>
			<div>
				테마별 <select name="lctCl">
					...
					<c:forEach items="${datas }" var="obj">
						<option value="${obj.lctCl }"
							${param.lctCl eq obj.lctCl ? 'selected' : ''}>${obj.lctCl}</option>
					</c:forEach>
				</select>
			...
			<tr onclick="location.href='/detail?contentId=${list.contentId}'">
				...
				<c:choose>
					<c:when test="${not empty list.firstImageUrl }">
						<td><img src="${list.firstImageUrl }"
							style="height: 100px; width: 120px"
							onerror="this.src='/resource/image/tent.png';" /></td>
					</c:when>
					<c:otherwise>
						<td><img src="/resource/image/tent.png"
							style="height: 100px; width: 120px" /></td>
					</c:otherwise>
				</c:choose>
				<td>${list.facltNm.replace("(주)", " ")}</td>
				<td>${list.lineIntro }(${list.doNm},${list.sigunguNm},
					${list.lctCl})</td>
				<td>${list.addr1}</td>
			</tr>
		</c:forEach>
